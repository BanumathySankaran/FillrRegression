...
import com.mop.qa.test.Utilities.TestParameter;
...
/*@Test
	...
	public static List<List<TestParameter>> testList() {
		...
		try {
			String platFormNameForNotepad = null;
			String udidNotepd = null;
			String deviceNameNotepd = null;
			...
			FileInputStream fis = new FileInputStream("./TestRunnerUAFiOS.xls");
				...
				for (int count = 1; count <= sheet.getLastRowNum(); count++) {
					...
					if (row.getCell(2).toString().equalsIgnoreCase("Yes")) {
						if (!row.getCell(6).toString().equalsIgnoreCase("Connected Device")) {// Get browsername an
							...
							platFormNameForNotepad = platform[0];
						...
						for (int l = 0; l < count1; l++) {// count as no of instances
							...
							if (row.getCell(6).toString().equalsIgnoreCase("Connected Device")) {
								...
								if (test.getToolName().equalsIgnoreCase("Appium")) {
									...
									deviceNameNotepd = deviceNames[l];
								}
			...
			osw.write(platFormNameForNotepad);
			osw.write(",udid".concat(udidNotepd));
			osw.write(",deviceName".concat(deviceNameNotepd));
			osw.close();
			return testRunner;
		} catch (Exception e) {
			e.printStackTrace();
